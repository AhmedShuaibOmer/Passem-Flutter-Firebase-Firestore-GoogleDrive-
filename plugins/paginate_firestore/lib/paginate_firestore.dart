...
export 'bloc/pagination_listeners.dart';
...
class _PaginateFirestoreState extends State<PaginateFirestore>
    with AutomaticKeepAliveClientMixin<PaginateFirestore> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.query != _oldQuery) {
      _oldQuery = widget.query;
      _bloc.add(PaginationFetchListRequested(query: widget.query));
    }
        ...
        if (listener is PaginateRefreshedChangeListener) {
          listener.addListener(() {
            if (listener.refreshed) {
              _bloc.add(PaginationRefreshListRequested());
            }
          });
        } else if (listener is PaginateQueryChangeListener) {
          listener.addListener(() {
            _bloc.add(PaginationFetchListRequested(query: listener.query));
          });
        } else if (listener is PaginateFilterChangeListener) {
          listener.addListener(() {
            if (listener.searchTerm.isNotEmpty) {
              _bloc.add(PaginationFilterListRequested(listener.searchTerm));
            }
          });
        }
      }
            ...
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                if (index >= loadedState.documentSnapshots.length) {
                  _bloc.add(PaginationFetchListRequested());
              ...
              (context, index) {
                ...
                if (index.isEven) {
                  if (itemIndex >= loadedState.documentSnapshots.length) {
                    _bloc
                        .add(PaginationFetchListRequested());
                    return widget.bottomLoader;
                  }
